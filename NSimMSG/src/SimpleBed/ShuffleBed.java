...
public class ShuffleBed {
	...
	private boolean startNxtFlowAt(int i) {
		...
		for (int k = 0 ; k < subflow; k++) {
			nowtcpc[i][k] = new TCPBony(here,(short) (i*H*subflow+nowflowid[i]*subflow+k),flowsizeKB);
			TCPBony RmTCP = new TCPBony(remote,(short) (i*H+nowflowid[i]+k));
	...
	private int[] permuWithoutMe(int me) {
		...
		for (int i = 1 ; i < H-1; i++) {
			...
			int t = permu[rand]; permu[rand]=permu[i]; permu[i] = t;
		}
		return permu;
	}
	...
	private boolean finished(TCPBony[] tcpBonies) {
		for (TCPBony s : tcpBonies)
			if (!s.finished()) return false;
		return true;
	}
	private double getAmount(TCPBony[] tcpBonies) {
		double ans = 0.0;
		for (TCPBony s : tcpBonies){
			ans += s.getAmount();
		}
