...
public class MSG_Topology {
	...
	public void printLinks_old() {
		...
		for (int i = 0 ; i< links.size(); i++) {
			pardists[i] = poscor(links.get(i).getUpNode().getCoor() ,links.get(i).getDownNode().getCoor());  
			id[i] = i;
		}
		for(int i = 0 ; i < links.size(); i++)
			for (int j = i+1; j< links.size(); j++)
				if (pardists[id[i]] > pardists[id[j]]){
					int tti = id[i]; id[i] = id[j]; id[j]=tti;
				}
	...
	public void printLinks_old2() {
		for (int i = 0; i < links.size(); i++) {
			Link l = links.get(i);
			Coor1D x1 = (Coor1D) links.get(i).getUpNode().getCoor();
			Coor1D x2 = (Coor1D) links.get(i).getDownNode().getCoor();
	...
	public void printLinks() {
		for (Switch s : nodes){
			...
			for (int dim = 0 ; dim < this.dimentions; dim++) {
				...
				for (int i = 0 ; i < nc; i++) 
					cl[i] =((Coor1D) vn.get(i).getCoor()).switchcoor[dim];
