...
public class TCPBony implements Support.Entity {
	...
	public TCPBony(Host parent, short port) {
		this.parent = parent;
	...
	public TCPBony(Host parent, short port, int filesize) {
		this(parent, port);
...
//		System.out.println(this.getName() + " Retransmit " + m.getSeq() + "to "+this.peer.getName());
	...
	public boolean receive(TCPMessage received) {
		...
		if (active) {
			...
			TCPMessage m = new TCPMessage(28, parent.getCoor(), peer.parent.getCoor(), port, peer.port, TTL,parent.id,peer.parent.id,flowid);
			m.setAck(LAS);
			m.setTcpdst(peer);
			NetworkEvent nev = new NetworkEvent(NetworkEvent.SEND, m, Simusys.time(), this);
			parent.addEvent(nev);
	...
	private void fastRetransmit(int ack) {
		TCPMessage m = new TCPMessage(PACKET_SIZE, parent.getCoor(), peer.parent.getCoor(), port, peer.port, TTL,parent.id,peer.parent.id,flowid);
		m.setSeq(ack + 1);
		m.setTcpdst(peer);
		retransmit(m);
	}
	...
	public Coordinate getCoor() {		return parent.getCoor();	}
